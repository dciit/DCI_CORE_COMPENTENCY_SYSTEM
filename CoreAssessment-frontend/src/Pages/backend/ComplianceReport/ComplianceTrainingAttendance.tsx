...
import { getDept, getSectionByDept } from "../../../service/admin";
import { IGroup, IEmployee } from "../../../Model/Admin/employeeAttendance";
function ComplianceTrainingAttendance() {
  ...
  const selectedDept = async (item: string) => {
    let oGroup: any = await getSectionByDept(item);
    setgroupList(oGroup);
  };
                                ...
                                <td
                                  className={` text-left p-1 w-[25%] ${
                                    emp.status == "ประเมินแล้ว"
                                      ...
                                      : (emp.status == "รอการ Confirm") ? " bg-yellow-300 text-black"
                                      :'text-red-500'
                                  }`}
                                >
                                  {emp.status}
...
export default ComplianceTrainingAttendance;
